<script lang="ts">
  ...
  // Form State
  let contractNumber = $state(`CTR-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`);
  let title = $state('');
  let clientId = $state('');
  ...
  onMount(async () => {
    try {
      ...
      console.error('Errore caricamento dati creazione contratto:', e);
    } finally {
      loading = false;
    }
  });
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      errorMsg = err.message || 'Errore durante la creazione del contratto.';
    } finally {
      saving = false;
    }
  }
    ...
    <form onsubmit={handleSubmit} class="contract-form">
      ...
      <div class="card form-card">
        ...
        <div class="grid-3 mb-16">
          ...
          <div class="form-group">
            <label for="contract-status">Stato Iniziale</label>
      ...
      <div class="form-actions-bar">
        <a href="/dashboard/contracts" class="btn-cancel">Annulla</a>
