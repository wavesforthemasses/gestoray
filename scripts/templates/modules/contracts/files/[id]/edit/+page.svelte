<script lang="ts">
  ...
  onMount(async () => {
    try {
      const [s, cList, uList, cf, data] = await Promise.all([
      ...
      if (!contract) {
        errorMsg = `${labels.singular} non trovato o rimosso.`;
        return;
      }
      ...
      else if ((rawType as string) === 'Fornitura / Quotazione' || (rawType as string) === 'Monte Ore' || (rawType as string) === 'SLA Garantito' || (rawType as string) === 'Licenza / Abbonamento') type = 'Non Ricorrente';
      else type = rawType;
      ...
      termsAndConditions = contract.termsAndConditions || s.defaultTermsAndConditions || '';
      tags = contract.tags || [];
      items = contract.items || [];
      discountType = contract.discountType || 'percent';
      discountValue = contract.discountValue || 0;
      customFieldsValues = contract.customFields || {};
      ...
      console.error('Errore caricamento contratto:', e);
      errorMsg = `Impossibile caricare i dati del ${labels.singular.toLowerCase()}.`;
    } finally {
      loading = false;
    }
  });
...
<div class="edit-contract-container animate-fade-in">
  ...
  {:else}
    <Card
      title={`Modifica ${contractNumber || labels.singular}`}
      description="Modifica le informazioni del documento e salva le modifiche."
      class="form-card"
    >
      {#snippet icon()}
      ...
      <form onsubmit={handleSubmit} class="contract-form-grid">
        ...
        <button type="submit" class="save-btn" disabled={saving}>
          ...
          {:else}
            Salva Modifiche {labels.singular}
          {/if}
        </button>
      </form>
    </Card>
