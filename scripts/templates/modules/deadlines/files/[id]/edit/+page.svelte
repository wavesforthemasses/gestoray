<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      const [s, data, uList] = await Promise.all([
      ...
      if (data) {
        ...
        recurringIntervalMonths = data.recurringIntervalMonths || 12;
        notes = data.notes || '';
        linkedEntityType = data.linkedEntityType || 'vehicle';
        linkedEntityId = data.linkedEntityId || '';
        linkedEntityName = data.linkedEntityName || '';
        pageTitle.set(`Modifica ${data.code}`);
      }
    } catch (e) {
      console.error('Errore caricamento dati scadenza:', e);
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    saving = true;
    try {
      await DeadlinesService.updateDeadline(deadlineId, {
        title: title.trim(),
        category,
      ...
      toast.success(`${labels.singular} aggiornata con successo!`);
      goto(`/dashboard/deadlines/${deadlineId}`);
    } catch (err: any) {
...
<div class="edit-deadline-container">
  {#if loading}
    ...
    <header class="page-header">
      <div class="header-title-box">
        ...
        <div>
          ...
          <p class="page-main-subtitle">Aggiorna le informazioni della scadenza ed il relativo stato.</p>
        </div>
      </div>
    </header>
      ...
      <div class="form-layout">
        <Card class="form-card">
          ...
          <div class="form-actions">
            <a href={`/dashboard/deadlines/${deadlineId}`} class="btn-cancel">Annulla</a>
