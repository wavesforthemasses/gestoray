<script lang="ts">
  ...
  import { ProjectSettingsService } from '../../projects/projectSettingsService';
  import type { ProjectSettings, ProjectStatus } from '../../projects/schema';
  import { toast } from '$lib/stores/toast.svelte';
  import { FolderKanban, Save, Layers, Hash, Calendar, CheckCircle2 } from '@lucide/svelte';
  ...
  onMount(async () => {
    try {
      settings = await ProjectSettingsService.getSettings();
    } catch (e) {
      console.error('Errore caricamento impostazioni progetti:', e);
    } finally {
      loading = false;
    }
  });
  ...
  async function handleSave() {
    saving = true;
    try {
      await ProjectSettingsService.saveSettings(settings);
...
<div class="settings-page-container">
  <header class="settings-header">
    <div class="header-title-group">
      ...
      <div>
        <h1 class="page-title">Configurazione {labels.plural} / Contenitori Agnostici</h1>
        <p class="page-subtitle">Personalizza denominazione agnostica, prefissi di codifica automatica e stati di default.</p>
      </div>
    </div>
  ...
  {:else}
    <div class="settings-grid">
      ...
      <div class="settings-card">
        ...
        <p class="card-desc">Scegli come deve essere chiamato questo contenitore nell'intero sistema (es. Progetto, Cantiere, Commessa, Pratica).</p>
        ...
        <div class="form-group">
          ...
          <select id="entityNaming" bind:value={settings.entityNaming} class="form-select">
            <option value="progetto">Progetto (Progetti)</option>
            <option value="cantiere">Cantiere (Cantieri)</option>
            <option value="commessa">Commessa (Commesse)</option>
            <option value="pratica">Pratica (Pratiche)</option>
      ...
      <div class="settings-card">
        ...
        <p class="card-desc">Definisci il formato del codice univoco generato automaticamente al momento dell'apertura.</p>

        <div class="form-group">
          <label for="prefix">Prefisso Codice</label>
          <input type="text" id="prefix" bind:value={settings.prefix} class="form-input" placeholder="es. PROG- o CANTIERE-" />
        </div>
      ...
      <div class="settings-card">
        ...
        <p class="card-desc">Stato assegnato automaticamente ai nuovi contenitori creati.</p>
