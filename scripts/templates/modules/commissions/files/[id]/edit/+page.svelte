<script lang="ts">
  ...
  // Form State
  let commissionNumber = $state('');
  let agentUid = $state('');
  let dealTitle = $state('');
  let dealAmount = $state<number>(0);
  let commissionRate = $state<number>(0);
  let earnedDate = $state('');
    ...
    try {
      ...
      if (commissionId) {
        ...
        if (commission) {
          ...
          agentUid = commission.agentUid || '';
          dealTitle = commission.dealTitle || '';
          dealAmount = commission.dealAmount || 0;
          commissionRate = commission.commissionRate || 0;
          earnedDate = commission.earnedDate || '';
          status = commission.status || 'maturata';
          notes = commission.notes || '';
          customFieldsValues = commission.customFields ? { ...commission.customFields } : {};
        }
      }
    } catch (e) {
      console.error('Errore caricamento dati modifica provvigione:', e);
    } finally {
      loading = false;
    }
  });
  ...
  async function handleSubmit(e: SubmitEvent) {
    e.preventDefault();
    if (!commissionId || !agentUid || !dealTitle.trim()) {
    ...
    try {
      ...
      await CommissionsService.updateCommission(commissionId, {
        commissionNumber: commissionNumber.trim(),
        agentUid,
        agentName: selectedAgent ? selectedAgent.name : (commission?.agentName || ''),
      ...
      toast.success('Provvigione aggiornata con successo!');
      goto(`/dashboard/commissions/${commissionId}`);
    } catch (err: any) {
...
<svelte:head>
  <title>Modifica Provvigione | {$projectStore?.projectName || 'ERP'}</title>
</svelte:head>

<div class="add-commission-page animate-fade-in">
  <div class="page-top">
    <a href="/dashboard/commissions/{commissionId}" class="back-link">← Torna al Dettaglio Provvigione</a>
    <h2>✏️ Modifica Provvigione Agente</h2>
  </div>
  ...
  {:else}
    ...
    <form onsubmit={handleSubmit} class="commission-form">
        ...
        <div class="grid-2 mb-16">
          ...
          <div class="form-group">
            ...
            <select id="comm-status" bind:value={status} class="form-control">
              ...
              <option value="stornata">↩️ Stornata</option>
            </select>
          </div>
        </div>
      ...
      <div class="form-actions-bar">
        <a href="/dashboard/commissions/{commissionId}" class="btn-cancel">Annulla</a>
        <button type="submit" class="btn-submit" disabled={saving}>
          {saving ? 'Salvataggio...' : '💾 Aggiorna Provvigione'}
        </button>
      </div>
    </form>
  {/if}
