<script lang="ts">
  ...
  import type { ProductType, ProductUsageType, BillingType, RecurrenceInterval } from '../schema';
  ...
  let purchasePrice = $state<number>(15);
  let unit = $state('pz');
  ...
  let minStockThreshold = $state<number>(2);
  ...
  function handleTypeSelect(type: ProductType) {
    ...
    if (type === 'product') {
      ...
      trackStock = false;
      usageType = 'sale'; // Services default to Sale only
      unit = 'ora';
      billingType = 'hourly';
      category = 'Servizi & Manodopera';
    } else if (type === 'digital') {
      trackStock = false;
      usageType = 'sale';
      unit = 'pz';
      billingType = 'one_off';
      category = 'Software & Licenze';
    }
  }
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      toast.success('Articolo aggiunto al catalogo con successo!');
      goto(`/dashboard/products/${prodId}`);
    } catch (err: any) {
      console.error('Errore salvataggio articolo:', err);
      errorMsg = err.message || 'Errore durante la creazione dell\'articolo.';
    } finally {
      saving = false;
    }
  }
...
<svelte:head>
  <title>Nuovo Articolo Catalogo | {$projectStore?.projectName || 'ERP'}</title>
</svelte:head>
    ...
    <form onsubmit={handleSubmit} class="product-form">
      ...
      <div class="card form-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">Scegli la natura dell'offerta per configurare automaticamente i parametri ideali.</p>
        </div>
      ...
      <div class="card form-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">Determina in quali moduli l'articolo è disponibile (Contratti/Preventivi o Ordini Fornitore).</p>
        </div>
        ...
        <div class="usage-cards-grid">
          ...
          >
            ...
            <div class="usage-content">
              ...
              <div class="usage-desc">Articolo commerciale standard comprato da fornitori e venduto ai clienti.</div>
            </div>
            {#if usageType === 'both'}
          ...
          >
            ...
            <div class="usage-content">
              ...
              <div class="usage-desc">Servizi, consulenze o kit offerti solo nei preventivi/contratti (non acquistati con PO).</div>
            </div>
            {#if usageType === 'sale'}
              <CheckCircle2 size={16} class="check-badge" />
            {/if}
          ...
          >
            ...
            <div class="usage-content">
              ...
              <div class="usage-desc">Materie prime, consumabili cantiere o cancelleria acquistati da fornitori (non a listino clienti).</div>
            </div>
      ...
      <div class="card form-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">Denominazione, codice univoco, listino vendita e costo d'acquisto.</p>
        </div>
      ...
      <!-- 5. GESTIONE SCORTE & MAGAZZINO (DECOUPLED) -->
      <div class="card form-card">
      ...
      <div class="form-actions">
        <a href="/dashboard/products" class="btn btn-secondary">Annulla</a>
        <button type="submit" class="btn btn-primary" disabled={saving}>
          <Save size={16} /> {saving ? 'Salvataggio...' : 'Crea Articolo'}
        </button>
      </div>
    </form>
  {/if}
