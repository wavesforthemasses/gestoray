...
function resetActiveWorkspace() {
  ...
  // 7. Reset firestore.indexes.json to Clean Base Core
  const cleanIndexes = {
    indexes: [
      {
        ...
        fields: [
          { fieldPath: 'original.createdBy', order: 'ASCENDING' },
          { fieldPath: 'edits.createdAt', order: 'ASCENDING' }
        ]
      },
      {
        collectionGroup: 'clients',
        ...
        fields: [
          { fieldPath: 'original.createdBy', order: 'ASCENDING' },
          { fieldPath: 'derived.nncfDate', order: 'ASCENDING' }
        ]
      },
      {
        collectionGroup: 'clients',
        ...
        fields: [
          { fieldPath: 'original.createdBy', order: 'ASCENDING' },
          { fieldPath: 'edits.createdAt', order: 'DESCENDING' }
        ]
      },
      {
        collectionGroup: 'clients',
        queryScope: 'COLLECTION',
        fields: [
          { fieldPath: 'derived.textSearch', arrayConfig: 'CONTAINS' },
          { fieldPath: 'edits.createdAt', order: 'DESCENDING' }
        ]
      }
    ],
  ...
  fs.writeFileSync(indexesPath, JSON.stringify(cleanIndexes, null, 2), 'utf-8');
  console.log(`  ✅ Ripristinato firestore.indexes.json al Base Core.`);
    ...
    function notWritingDerived() {
      return (request.resource == null) ||
             ...
             ((resource == null || !('derived' in resource.data)) && 
                request.resource.data.derived.keys().hasOnly(['textSearch', 'cacheChunkId', 'activitiesCount', 'placesCount', 'contractCount', 'ticketsCount', 'deleted'])) ||
             (resource != null && ('derived' in resource.data) && (
                request.resource.data.derived == resource.data.derived ||
                request.resource.data.derived.diff(resource.data.derived).affectedKeys().hasOnly(['textSearch', 'cacheChunkId', 'activitiesCount', 'placesCount', 'contractCount', 'ticketsCount', 'deleted'])
             ));
    }
...
`;
  ...
  console.log(`\n✨ RESET COMPLETATO! Il sistema è ora in stato Clean Base Core.`);
