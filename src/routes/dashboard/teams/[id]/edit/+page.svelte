<script lang="ts">
  ...
  onMount(async () => {
    try {
      if (!teamId) return;
      const [s, data, usersSnap] = await Promise.all([
      ...
      if (data) {
        ...
        members = (data.members || []).map(m => ({
          ...m,
          isLeader: m.isLeader || m.userId === data.leaderId
        }));
        status = data.status;
        notes = data.notes || '';
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      goto(`/dashboard/teams/${teamId}`);
    } catch (e) {
      console.error('Errore aggiornamento squadra:', e);
    ...
    <header class="page-header">
      <div class="header-title-box">
        ...
        <div>
          ...
          <p class="page-main-subtitle">Aggiorna le informazioni della squadra, l'eventuale mezzo ed i membri del team.</p>
        </div>
      </div>
    </header>
    ...
    <form onsubmit={handleSubmit}>
      <div class="form-layout">
        ...
        <Card class="form-card">
          ...
          <p class="section-desc">Aggiungi o rimuovi gli operatori assegnati e spunta chi è il **Caposquadra**.</p>
          ...
          <div class="form-actions">
            <a href={`/dashboard/teams/${teamId}`} class="btn-cancel">Annulla</a>
