<script lang="ts">
  ...
  async function handleUpdateProfile(e: Event) {
    ...
    try {
      ...
      const fields = ['nome', 'cognome', 'email', 'phone'];
  ...
  async function logActivity(type: 'Telefonata' | 'Incontro' | 'Appuntamento', datetimeVal?: string) {
    if (!$auth) return;
            ...
            <Card title="Scheda Anagrafica Cliente" description="Aggiorna le informazioni di contatto principali. Ogni modifica viene tracciata nell'Audit Log.">
              ...
              <form onsubmit={handleUpdateProfile} class="widescreen-form">
                <div class="form-grid-columns">
                  <FormField id="c-nome" label="Nome / Nome Azienda">
                    <input type="text" id="c-nome" bind:value={clientName} required disabled={submittingProfile} />
                  </FormField>

                  <FormField id="c-cognome" label="Cognome Referente" helpText="Opzionale">
                    <input type="text" id="c-cognome" bind:value={clientCognome} disabled={submittingProfile} />
                  </FormField>
                </div>

                <div class="form-grid-columns">
                  <FormField id="c-email" label="Indirizzo Email" helpText="Opzionale">
                    <input type="email" id="c-email" bind:value={clientEmail} placeholder="es. client@azienda.com" disabled={submittingProfile} />
                  </FormField>

                  <FormField id="c-phone" label="Numero di Telefono" helpText="Opzionale">
                    <input type="text" id="c-phone" bind:value={clientPhone} placeholder="es. +39 02 123456" disabled={submittingProfile} />
                  </FormField>
                </div>
                ...
                {:else}
                  ...
                  <div class="timeline-flow">
                    {#each timelineItems as item}
                      <div class="timeline-card" class:note-item={item.source === 'note'}>
                        <div class="card-top">
