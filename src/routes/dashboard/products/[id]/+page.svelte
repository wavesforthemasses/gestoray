...
<div class="product-detail-page animate-fade-in">
  ...
  {:else if !product}
    <div class="alert error-box">⚠️ Articolo non trovato o eliminato.</div>
  {:else}
    <!-- HEADER -->
    <header class="detail-header card">
      ...
      <div class="header-actions">
        <button type="button" class="btn btn-secondary" onclick={printDetails}>🖨️ Stampa Scheda</button>
        <a href="/dashboard/products/{productId}/edit" class="btn btn-secondary">✏️ Modifica Articolo</a>
      </div>
    </header>

    <!-- INFO CARD -->
    <div class="card info-card">
      <h3 class="card-title">ℹ️ Prezzo & Giacenza Magazzino</h3>
...
<style>
  ...
  .stock-badge { font-size: 0.85rem; font-weight: 700; padding: 0.2rem 0.6rem; border-radius: 6px; }
