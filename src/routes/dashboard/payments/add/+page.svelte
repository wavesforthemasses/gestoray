<script lang="ts">
  ...
  import type { PaymentMethod, PaymentStatus } from '../schema';
  ...
  // Form State
  let paymentNumber = $state(`INC-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`);
  let clientId = $state('');
  let amount = $state<number>(450);
...
<div class="add-payment-page animate-fade-in">
  <div class="page-top">
    ...
    <h2>💶 Registra Nuovo Incasso Clienti</h2>
  </div>
    ...
    <form onsubmit={handleSubmit} class="payment-form">
      ...
      <div class="card form-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">Seleziona il cliente per cui stai registrando l'incasso.</p>
        </div>
      ...
      <div class="form-actions-bar">
        <a href="/dashboard/payments" class="btn-cancel">Annulla</a>
...
<style>
  .add-payment-page { max-width: 900px; margin: 0 auto; padding: 24px 16px; }
